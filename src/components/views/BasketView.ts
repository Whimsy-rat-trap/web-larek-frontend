...
export class BasketView {
	...
	addItem(product: Product): void {
		if (product.price === null) {
			console.warn(
				`Товар "${product.title}" не может быть добавлен в корзину - цена не указана`
			);
			return;
		}
	...
	private updateBasket(): void {
		...
		const checkoutButton = basketElement.querySelector(
			'.button'
		) as HTMLButtonElement; // Находим кнопку оформления
		...
		if (basketList && basketPrice) {
			...
			this.items.forEach((item, index) => {
				const basketItem = new BasketItemView(this.basketItemTemplate, {
					onDelete: (productId: string) => this.removeItem(productId),
				});

				const basketItemElement = basketItem.render(item, index);
